...
class StorageService: ObservableObject {
    ...
    func saveCard(_ card: Card) async throws {
        ...
        // Bind JSON arrays as text
        let tagsJSON = String(data: try JSONEncoder().encode(card.tags), encoding: .utf8) ?? "[]"
        sqlite3_bind_text(statement, 8, (tagsJSON as NSString).utf8String, -1, nil)

        let networkIdsJSON = String(data: try JSONEncoder().encode(card.networkIds), encoding: .utf8) ?? "[]"
        sqlite3_bind_text(statement, 9, (networkIdsJSON as NSString).utf8String, -1, nil)
    ...
    private func loadCards() async throws {
        ...
        while sqlite3_step(statement) == SQLITE_ROW {
            guard let stmt = statement else {
                print("⚠️ Statement is nil")
                continue
            }
            do {
                let card = try parseCardRow(stmt, masterKey: masterKey)
    ...
    private func parseCardRow(_ statement: OpaquePointer, masterKey: SymmetricKey) throws -> Card {
        guard let idText = sqlite3_column_text(statement, 0),
              let userIdText = sqlite3_column_text(statement, 1),
              let nameText = sqlite3_column_text(statement, 2),
              let barcodeTypeText = sqlite3_column_text(statement, 3) else {
            throw StorageError.sqliteError("Failed to read card data")
        }

        let id = String(cString: idText)
        let userId = String(cString: userIdText)
        let name = String(cString: nameText)
        let barcodeTypeRaw = String(cString: barcodeTypeText)

        guard let barcodeType = BarcodeType(rawValue: barcodeTypeRaw) else {
            throw StorageError.sqliteError("Invalid barcode type: \(barcodeTypeRaw)")
        }

        // Get encrypted data
        guard let ciphertextBytes = sqlite3_column_blob(statement, 4) else {
            throw StorageError.sqliteError("Missing ciphertext data")
        }
        let ciphertextLen = sqlite3_column_bytes(statement, 4)
        let ciphertext = Data(bytes: ciphertextBytes, count: Int(ciphertextLen))

        guard let nonceBytes = sqlite3_column_blob(statement, 5) else {
            throw StorageError.sqliteError("Missing nonce data")
        }
        let nonceLen = sqlite3_column_bytes(statement, 5)
        let nonce = Data(bytes: nonceBytes, count: Int(nonceLen))

        guard let tagBytes = sqlite3_column_blob(statement, 6) else {
            throw StorageError.sqliteError("Missing tag data")
        }
